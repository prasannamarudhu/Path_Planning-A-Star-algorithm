...
def create_graph(resol):
    ...
    xy = np.array([[(round(125/resol)), (round(56/resol))], [(round(150/resol)), (round(15/resol))], [(round(173/resol)), (round(15/resol))], [(round(163/resol)), (round(52/resol))], [(round(170/resol)), (round(90/resol))], [(round(193/resol)), (round(52/resol))]])
    ...
    for i in range(round(250 / resol)):
        for j in range(round(150 / resol)):
            ...
            if ((i - (round(190 / resol))) ** 2 + (j - (round(130 / resol))) ** 2 - r ** 2) <= 0:
                graph[(i, j)]['valid'] = False
            if (((i - (round(140 / resol))) ** 2) / (a ** 2) + ((j - (round(120 / resol))) ** 2) / (
                    b ** 2) - 1) <= 0:
                graph[(i, j)]['valid'] = False
            if ((A1[0] * i + B1[0] * j + C1[0]) <= 0) and ((A1[1] * i + B1[1] * j + C1[1]) <= 0) and (
                    (A1[2] * i + B1[2] * j + C1[2]) <= 0) and ((A1[3] * i + B1[3] * j + C1[3]) <= 0):
                graph[(i, j)]['valid'] = False
            if (A2[0] * i + B2[0] * j + C2[0] <= 0) and (A2[1] * i + B2[1] * j + C2[1] <= 0) and (
                    A2[2] * i + B2[2] * j + C2[2] <= 0) and (A2[3] * i + B2[3] * j + C2[3] <= 0):
                graph[(i, j)]['valid'] = False
            if (A3[0] * i + B3[0] * j + C3[0] <= 0) and (A3[1] * i + B3[1] * j + C3[1] <= 0) and (
                    A3[2] * i + B3[2] * j + C3[2] <= 0) and (A3[3] * i + B3[3] * j + C3[3] <= 0):
                graph[(i, j)]['valid'] = False
...
def calculate_distance(goal, current):
    ...
    d = w*math.sqrt(((goal[0]-current[0])*(goal[0]-current[0]))+((goal[1]-current[1])*(goal[1]-current[1])))
    return d
...
def Neighbor_NodeCreate(Node_Current, goal_node, obstacle_points, resol_grid):
    ...
    for i in range(0, 8):
        ...
        Heuristic_cost = cost_new + Distance_Heuristic(Gen_X, Gen_Y, Goal_X, Goal_Y) #Calculating the heuristic cost for each possible nodes
        Pos = (Gen_X, Gen_Y)
...
def user_input():
    ...
    resol = input("Enter the resolution value : ")
    gx = input("Enter the Goal node 'X' value : ")
    gy = input("Enter the Goal node 'Y' value : ")

    return int(sx), int(sy),int(resol),int(gx), int(gy)
...
if __name__ == "__main__":
    ...
    sx, sy, resol, gx, gy = user_input()
    sx = (round(sx / resol))  #Rounding off the values to handle the resolution of the map
    sy = (round(sy / resol))
    gx = (round(gx / resol))
    gy = (round(gy / resol))

    #Negative values for node positions not allowed
    if sx < 0 or sy < 0 or gx < 0 or gy < 0:
        print("\n\nNegative values for node positions not allowed")
        exit()

    #Creating the Graph
    g = create_graph(resol)
    ...
    #ax.set_xlim(0, 250)
    #ax.set_ylim(0, 150)
    plt.xlim(0, round(250 / resol))
    plt.ylim(0, round(150 / resol))
    plt.title("A-Star Simulation for Point Robot")
